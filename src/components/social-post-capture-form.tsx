...
const emptyDraft: SocialPostEventDraft = {
  title: "",
  description: "",
  category: "other",
  citySlug: "karachi",
...
export interface SocialPostCaptureConfig {
  source: "linkedin" | "instagram";
  extractApiPath: string;
  platformLabel: string;
  postUrlLabel: string;
  postUrlPlaceholder: string;
  postTextLabel: string;
  postTextPlaceholder: string;
  sourcePostUrlLabel: string;
  sourcePostUrlPlaceholder: string;
  submitLabel: string;
  reviewNote: string;
}

export function SocialPostCaptureForm({
  cities,
  config,
}: {
  cities: SubmitCityOption[];
  config: SocialPostCaptureConfig;
}) {
  const [postUrl, setPostUrl] = useState("");
  const [postText, setPostText] = useState("");
  const [draft, setDraft] = useState<SocialPostEventDraft>(emptyDraft);
  ...
  async function handleExtract() {
    ...
    try {
      const response = await fetch(config.extractApiPath, {
        method: "POST",
    ...
    <section className="grid gap-8 lg:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)]">
      <div className="h-fit border-2 border-foreground bg-card">
        <div className="border-b-2 border-foreground p-5 sm:p-7">
          <p className="flex items-center gap-2 font-mono text-[10px] font-semibold uppercase tracking-[0.16em] text-primary">
            ...
            {config.platformLabel} capture
          </p>
          <h2 className="mt-3 font-heading text-4xl font-semibold tracking-[-0.04em]">Paste the signal.</h2>
          <p className="mt-3 text-sm leading-relaxed text-muted-foreground">
            Use a public {config.platformLabel} post URL, or paste the text when the platform asks for login.
          </p>
        </div>
        ...
        <div className="grid gap-5 p-5 sm:p-7">
          <div>
            ...
            <Input
              ...
              placeholder={config.postUrlPlaceholder}
              className={fieldClass}
            />
          </div>
          <div>
            <label htmlFor="postText" className={labelClass}>
              {config.postTextLabel}
            </label>
            <textarea
              id="postText"
              value={postText}
              ...
              rows={9}
              placeholder={config.postTextPlaceholder}
          ...
          {usedAi ? (
            <StatusBox kind="warning">AI extraction applied. Double-check dates, city, and registration link.</StatusBox>
          ) : null}
          {warning ? (
      ...
      >
        <div className="border-b-2 border-foreground p-5 sm:p-7">
          ...
          <h2 className="mt-2 font-heading text-4xl font-semibold tracking-[-0.04em]">Extracted event sheet</h2>
        </div>
        ...
        <div className="grid gap-5 p-5 sm:p-7">
          ...
          <div>
            ...
            <Input
              ...
              placeholder={config.sourcePostUrlPlaceholder}
              className={fieldClass}
            />
          </div>
          ...
          <div>
            ...
            <Input
              id="prizePool"
              name="prizePool"
              defaultValue={draft.prizePool}
              placeholder="PKR 500,000 / swag"
              className={fieldClass}
            />
          </div>
          ...
          <div className="flex flex-col gap-5 border-t border-foreground/35 pt-5 sm:flex-row sm:items-center sm:justify-between">
            <p className="text-xs text-muted-foreground">{config.reviewNote}</p>
            <Button
              type="submit"
              disabled={status === "submitting" || status === "extracting"}
              className="editorial-shadow h-12 rounded-none border border-foreground bg-primary px-6 text-xs font-semibold uppercase tracking-[0.12em] text-primary-foreground hover:-translate-x-0.5 hover:-translate-y-0.5 hover:bg-brand-hover"
            >
              {status === "submitting" ? "Filing..." : config.submitLabel}
              <ArrowRight />
            </Button>
          </div>
        </div>
